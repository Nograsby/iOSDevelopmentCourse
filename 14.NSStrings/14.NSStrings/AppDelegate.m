...
//  14.NSStrings
//
//  Created by Vladimir Ananko on 11/9/16.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSLog(@"Lenght of first part is - %d letters.\n%@\n ", firstPart.length, firstPart);
    NSLog(@"Lenght of second part is - %d letters.\n%@", secondPart.length, secondPart);

    NSArray *stringsArray = [text componentsSeparatedByString:@"."];
    NSLog(@"Array of strings.\n%@", stringsArray);
    
    // Student
    NSMutableArray *stringsArray2 = [NSMutableArray array];
    for (NSString *string in stringsArray) {
        if ([string containsString:@"NSString"]) {
            [stringsArray2 addObject:string];
        }
    }
    NSLog(@"Array of strings containing NSString.\n%@", stringsArray2);

    // Master
    NSMutableArray *stringsArray3 = [NSMutableArray array];
    for (NSString *string in stringsArray2) {
        NSArray *array = [string componentsSeparatedByString:@" "];
        NSArray *reversedArray = [[array reverseObjectEnumerator] allObjects];
        NSString *reversedString = [reversedArray componentsJoinedByString:@" "];
        [stringsArray3 addObject:reversedString];
    }
    NSLog(@"Array of reversed strings containing NSString.\n%@", stringsArray3);
    
    // Supermen
    NSString *tempString = [self stringWithLowerCaseFirstLetterInString:text];
    NSString *formattingText = [self stringWithUpperCaseLastLetterInString:tempString];
    
    NSLog(@"FormattingText   ---   %@", formattingText);

    return YES;
}

- (NSString *)stringWithLowerCaseFirstLetterInString:(NSString *)text {
    NSArray *stringsArray = [text componentsSeparatedByString:@" "];
    
    NSMutableArray *lowerCaseArray = [NSMutableArray array];
    for (NSString *string in stringsArray) {
        NSRange range = [self getFirstAlphabeticSymbolRangeInString:string];
        
        if (range.location != NSNotFound) {
            NSString *firstLetter = [string substringWithRange:range];
            NSString *lowerCaseString = [string stringByReplacingCharactersInRange:range withString:firstLetter.lowercaseString];
            [lowerCaseArray addObject:lowerCaseString];
        }
        else {
            [lowerCaseArray addObject:string];
        }
    }
    
    NSString *resultString = [lowerCaseArray componentsJoinedByString:@" "];
    
    return resultString;
}

- (NSString *)stringWithUpperCaseLastLetterInString:(NSString *)text {
    NSArray *stringsArray = [text componentsSeparatedByString:@" "];
    NSMutableArray *upperCaseArray = [NSMutableArray array];
    
    for (NSString *string in stringsArray) {
        NSString *reversedString = [self getReversedStringFromString:string];
        NSRange range = [self getFirstAlphabeticSymbolRangeInString:reversedString];
        
        if (range.location != NSNotFound) {
            NSString *firstLetter = [reversedString substringWithRange:range];
            NSString *upperCaseReversedString = [reversedString stringByReplacingCharactersInRange:range withString:firstLetter.uppercaseString];
            NSString *upperCaseString = [self getReversedStringFromString:upperCaseReversedString];
            [upperCaseArray addObject:upperCaseString];
        }
        else {
            [upperCaseArray addObject:string];
        }
        
    }
    NSString *resultString = [upperCaseArray componentsJoinedByString:@" "];
    return resultString;
}

- (NSRange)getFirstAlphabeticSymbolRangeInString:(NSString *)string {
    NSRegularExpression *expression = [NSRegularExpression regularExpressionWithPattern:@"[A-Za-z]"
                                                                                options:NSRegularExpressionCaseInsensitive
                                                                                  error:nil];
    NSRange range = [expression rangeOfFirstMatchInString:string
                                                  options:NSMatchingReportCompletion
                                                    range:NSMakeRange(0, string.length)];
    return range;
}

- (NSString *)getReversedStringFromString:(NSString *)string {
